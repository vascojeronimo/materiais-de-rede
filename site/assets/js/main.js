...
function Close(e) {
    ...
    const content = document.getElementsByClassName("menucontent")[0];
    const top = document.getElementsByClassName("mobiletop")[0];
    const scroll = document.getElementsByClassName("scroll");
    let stop = false

    if(container.style.display != "") {

        for(let testscroll in scroll) {
            if (e.target.parentNode == scroll[testscroll]) {
                stop = true
                break;
            };
        }
        ...
        if(e.target.parentNode == mobile || e.target.parentNode == menu || e.target.parentNode == container || e.target.parentNode == content || e.target.parentNode == top) return
...
window.onclick = e => Close(e);
window.ontouchend = e => Close(e)